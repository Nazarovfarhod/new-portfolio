import { IoLogoVercel } from "react-icons/io5";
import { FaGithub } from "react-icons/fa";

function Projects() {
  return (
    <div className="px-10 mb-20">
      <h2 className="text-4xl font-bold mb-5 lg:mb-20 tracking-[4px] lg:text-5xl">
        My Projects
      </h2>
      <ul className="flex flex-wrap gap-5 justify-center">
        <li className="flex flex-col items-center gap-5 shadow-white w-80 py-8 bg-[#222] rounded-xl">
          <h3 className="text-xl font-bold md:text-2xl lg:text-3xl">
            Tip Calculator
          </h3>
          <div className="flex gap-8">
            <a
              href="https://github.com/Nazarovfarhod/Calculator-of-tip.git"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <FaGithub className="w-8 h-8" />
              <span className="">GitHub</span>
            </a>
            <a
              href="https://calculator-of-tip.vercel.app/"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <IoLogoVercel className="w-8 h-8" />
              <span className="">Vercel</span>
            </a>
          </div>
        </li>
        <li className="flex flex-col items-center gap-5 shadow-white w-80 py-8 bg-[#222] rounded-xl">
          <h3 className="text-xl font-bold md:text-2xl lg:text-3xl">
            Devfinder
          </h3>
          <div className="flex gap-8">
            <a
              href="https://github.com/Nazarovfarhod/devfinder.git"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <FaGithub className="w-8 h-8" />
              <span className="">GitHub</span>
            </a>
            <a
              href="https://devfinder-pink-pi.vercel.app/"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <IoLogoVercel className="w-8 h-8" />
              <span className="">Vercel</span>
            </a>
          </div>
        </li>
        <li className="flex flex-col items-center gap-5 shadow-white w-80 py-8 bg-[#222] rounded-xl">
          <h3 className="text-xl font-bold md:text-2xl lg:text-3xl">
            FN Furniture
          </h3>
          <div className="flex gap-8">
            <a
              href="https://github.com/Nazarovfarhod/Furniture.git"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <FaGithub className="w-8 h-8" />
              <span className="">GitHub</span>
            </a>
            <a
              href="https://furniture-five-ashy.vercel.app/"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <IoLogoVercel className="w-8 h-8" />
              <span className="">Vercel</span>
            </a>
          </div>
        </li>
        <li className="flex flex-col items-center gap-5 shadow-white w-80 py-8 bg-[#222] rounded-xl">
          <h3 className="text-xl font-bold md:text-2xl lg:text-3xl">Quiz</h3>
          <div className="flex gap-8">
            <a
              href="https://github.com/Nazarovfarhod/Quiz.git"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <FaGithub className="w-8 h-8" />
              <span className="">GitHub</span>
            </a>
            <a
              href="https://quiz-dusky-psi.vercel.app/"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <IoLogoVercel className="w-8 h-8" />
              <span className="">Vercel</span>
            </a>
          </div>
        </li>
        <li className="flex flex-col items-center gap-5 shadow-white w-80 py-8 bg-[#222] rounded-xl">
          <h3 className="text-xl font-bold md:text-2xl lg:text-3xl">
            My Kitchen
          </h3>
          <div className="flex gap-8">
            <a
              href="https://github.com/Nazarovfarhod/My-kitchen.git"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <FaGithub className="w-8 h-8" />
              <span className="">GitHub</span>
            </a>
            <a
              href="https://my-kitchen-five.vercel.app/"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <IoLogoVercel className="w-8 h-8" />
              <span className="">Vercel</span>
            </a>
          </div>
        </li>
        <li className="flex flex-col items-center gap-5 shadow-white w-80 py-8 bg-[#222] rounded-xl">
          <h3 className="text-xl font-bold md:text-2xl lg:text-3xl">
            Products shop
          </h3>
          <div className="flex gap-8">
            <a
              href="https://github.com/Nazarovfarhod/Login-Logout.git"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <FaGithub className="w-8 h-8" />
              <span className="">GitHub</span>
            </a>
            <a
              href="https://login-logout-psi.vercel.app/"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <IoLogoVercel className="w-8 h-8" />
              <span className="">Vercel</span>
            </a>
          </div>
        </li>
        <li className="flex flex-col items-center gap-5 shadow-white w-80 py-8 bg-[#222] rounded-xl">
          <h3 className="text-xl font-bold md:text-2xl lg:text-3xl">
            Gues my number
          </h3>
          <div className="flex gap-8">
            <a
              href="https://github.com/Nazarovfarhod/Guss-my-number.git"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <FaGithub className="w-8 h-8" />
              <span className="">GitHub</span>
            </a>
            <a
              href="https://guss-my-number.vercel.app/"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <IoLogoVercel className="w-8 h-8" />
              <span className="">Vercel</span>
            </a>
          </div>
        </li>
        <li className="flex flex-col items-center gap-5 shadow-white w-80 py-8 bg-[#222] rounded-xl">
          <h3 className="text-xl font-bold md:text-2xl lg:text-3xl">
            Audio player.
          </h3>
          <div className="flex gap-8">
            <a
              href="https://github.com/Nazarovfarhod/Musics-Player.git"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <FaGithub className="w-8 h-8" />
              <span className="">GitHub</span>
            </a>
            <a
              href="https://musics-player-two.vercel.app"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <IoLogoVercel className="w-8 h-8" />
              <span className="">Vercel</span>
            </a>
          </div>
        </li>
        <li className="flex flex-col items-center gap-5 shadow-white w-80 py-8 bg-[#222] rounded-xl">
          <h3 className="text-xl font-bold md:text-2xl lg:text-3xl">
            Random User
          </h3>
          <div className="flex gap-8">
            <a
              href="https://github.com/Nazarovfarhod/random-user.git"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <FaGithub className="w-8 h-8" />
              <span className="">GitHub</span>
            </a>
            <a
              href="https://random-user-ashen-chi.vercel.app/"
              className="flex items-center gap-5 hover:text-[#0079FF] hover:underline transition-colors"
              target="_blank"
            >
              <IoLogoVercel className="w-8 h-8" />
              <span className="">Vercel</span>
            </a>
          </div>
        </li>
      </ul>
    </div>
  );
}

export default Projects;
